...
export function ItemRow({ item, onEdit, onDelete, onChangeOpenWith }: ItemRowProps) {
  ...
  const confirmDelete = () => { setShowDeleteConfirm(false); onDelete(item.id); };
  ...
  return (
    <>
      ...
      <Modal open={showDeleteConfirm} onClose={cancelDelete} onConfirm={confirmDelete} title={t("item.delete")}>
        <p>{t("item.deleteConfirm")}</p>
